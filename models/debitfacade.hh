...
class DebitFacade : public QObject
{
    ...
    using DebitType = DebitModel::DebitType;
    using RefreshRate = DebitModel::RefreshRate;
...
public:
    ...
    // QString transactionAmount(qint32 index) const {
    //     return QString::number(m_debitModel->transactionsList()->transaction(index)->amount());
    // }
    // QString transactionStartDate(qint32 index) const {
    //     return QString::number(m_debitModel->transactionsList()->transaction(index)->startDate());
    // }
    // QString transactionRefreshRate(qint32 index) const {
    //     return QString::number(m_debitModel->transactionsList()->transaction(index)->refreshRate());
    // }
    // QString transactionType(qint32 index) const {
    //     return QString::number(m_debitModel->transactionsList()->transaction(index)->type());
    // }
    ...
    void changeRefreshRate() {
        RefreshRate actualRefreshRate {m_debitModel->refreshRate()};
        if (actualRefreshRate == RefreshRate::Once) {
            m_debitModel->setRefreshRate(RefreshRate::Month);
        } else if (actualRefreshRate == RefreshRate::Month) {
            m_debitModel->setRefreshRate(RefreshRate::Quartal);
        } else if (actualRefreshRate == RefreshRate::Quartal) {
            m_debitModel->setRefreshRate(RefreshRate::Half);
        } else if (actualRefreshRate == RefreshRate::Half) {
            m_debitModel->setRefreshRate(RefreshRate::Year);
        } else if (actualRefreshRate == RefreshRate::Year) {
            m_debitModel->setRefreshRate(RefreshRate::Once);
        }
        // emit refreshRateChanged;
    }

    // void addTransaction(const Transaction& address) {
    //     m_debitModel->addTransaction(address);
    //     emit transactionListChanged;
    // }

    // void deleteTransaction(const Transaction& address) {
    //     m_debitModel->deleteTransaction(address);
    //     emit transactionListChanged;
    // }
